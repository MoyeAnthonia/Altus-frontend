// Usage:
//   import { DinoRunGame, DIFFICULTIES } from './dinoRunEngine.js';
//   const game = new DinoRunGame({
//     canvas: canvasEl,
//     difficulty: 'medium',       // one of: easy | medium | hard | score_attack
//     onGameEnd: (result) => {},  // called once on GAME_OVER or WIN
//   });
//   // ...
//   game.destroy(); // call on unmount to stop loops/timers
...
function drawText(ctx, text, x, y, size, color, align = 'left', baseline = 'top') {
...
function drawRect(ctx, x, y, w, h, color, strokeColor, strokeWidth) {
  ctx.fillStyle = color;
...
// ── Sprite drawing (pixel-block sprites) ───────────────────────────────────
function drawDino(ctx, x, y, frame, ducking, dead) {
...
function drawCactus(ctx, x, bottomY, type) {
...
export class DinoRunGame {
  /**
   * @param {Object} opts
   * @param {HTMLCanvasElement} opts.canvas - target canvas (will be sized to 800x300)
   * @param {'easy'|'medium'|'hard'|'score_attack'} [opts.difficulty='medium']
   * @param {(result: {result: 'won'|'lost', score: number, repStreak: number, timeMult: number, finalScore: number, secs: number}) => void} [opts.onGameEnd]
   */
  constructor(opts = {}) {
    ...
    this.reset();
    this.onIdle(); // applies initial difficulty immediately, no DIFFICULTY screen
  ...
  /** Change difficulty from outside (e.g. React re-render with a new prop). Only takes effect from IDLE. */
  setDifficulty(key) {
