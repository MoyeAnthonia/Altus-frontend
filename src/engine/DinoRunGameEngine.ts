...
class StateMachine {
  ...
  constructor(initial: StateName = "IDLE") {
...
export class DinoRunGame {
  ...
  private handleKeyDown(e: KeyboardEvent): void {
    ...
    if (e.code === "Space" || e.code === "ArrowUp") {
      ...
      if (this.sm.is(S.IDLE)) {
        this.sm.transition("CALIBRATING");
        return;
      }
      if (this.sm.is(S.GAME_OVER)) {
        this.restart();
        return;
      }
      if (this.sm.is(S.WIN)) {
  ...
  private onPaused(): void {
    if (this.obstacleTimerId) {
      clearTimeout(this.obstacleTimerId);
      this.obstacleTimerId = null;
    }
  }
  ...
  private update(dt: number): void {
    this.dino.ducking =
      !!this.keys.down && this.sm.is("ACTIVE") && Math.abs(this.dino.y - this.dinoGroundY) < 4;
    if (!this.sm.is("ACTIVE")) return;
    ...
    if (this.dino.frameTimer > 0.12) {
      this.dino.frameTimer = 0;
      this.dino.frame = this.dino.frame === 0 ? 1 : 0;
    }
    ...
    if (this.difficulty && this.score >= this.difficulty.repGoal) {
      this.sm.transition("WIN");
      return;
    }

    // Collision (AABB, inset for fairness)
    const dpad = 10;
    const dx = this.dino.x + dpad,
      dy = this.dino.y + dpad;
    const dw = this.dino.w - dpad * 2,
      dh = (this.dino.ducking ? 60 : this.dino.h) - dpad * 2;
  ...
  private scoreBreakdown(): ScoreBreakdown {
    ...
    const finalScore = Math.round(
      this.score * 100 * repStreak * timeMult * diffMult * closeCallMult,
    );
    return {
      repStreak,
      timeMult,
      ...
      finalScore,
      secs: Math.floor(this.sessionTime),
    };
  }
